...
export default function AdminStudentVerification() {
  ...
  return (
    <div>
      <h1 className="text-xl font-bold mb-1">{t("admin.verifyTitle")}</h1>
      <p className="text-sm text-gray-500 mb-6">{students.length} {t("admin.students").toLowerCase()}</p>
      <div className="space-y-2">
        {students.map(u=>(
          <div key={u.uid} className="card flex items-center justify-between gap-3">
            ...
            <button onClick={()=>handleVerify(u.uid)} className="text-xs px-2 py-1 rounded bg-green-100 text-green-700 hover:bg-green-200"><CheckCircle size={12} className="inline"/> {t("admin.verify")}</button>
          </div>
        ))}
      </div>
    </div>
